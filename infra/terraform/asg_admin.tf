...
resource "aws_launch_template" "admin" {
  ...
  image_id      = var.admin_ami_id != "" ? var.admin_ami_id : var.ami_id
  instance_type = var.admin_instance_type != "" ? var.admin_instance_type : var.instance_type
...
resource "aws_autoscaling_policy" "admin_cpu_target" {
  name                   = "${var.app_name}-${var.environment}-admin-cpu-target"
...
resource "aws_autoscaling_policy" "admin_alb_request_target" {
  name                   = "${var.app_name}-${var.environment}-admin-alb-request-target"
  autoscaling_group_name = aws_autoscaling_group.admin.name
  policy_type            = "TargetTrackingScaling"
  ...
  target_tracking_configuration {
    predefined_metric_specification {
      ...
      resource_label         = "${aws_lb.main.arn_suffix}/${aws_lb_target_group.admin.arn_suffix}"
    }
